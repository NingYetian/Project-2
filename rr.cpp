...
// visited.
  ...
  if((next.first < image.width()) && (next.second < image.height())){
    if(D[next.first][next.second]==-1){  //how to check unvisited???
      if(closeEnough(*image.getPixel(curr.first, curr.second),*image.getPixel(next.first, next.second))){  //how to change it to RGBAPixel?
        return true;
      }else{
        return false;
      }
...
vector<pair<int,int>> neighbors(pair<int,int> curr) {
  ...
  vector<pair<int,int>> n;  //vector of neighbours
...
// 33 = (9+1+4+1+3+1)+9+1+4.
  ...
  for(int i = 0; i<image.width(); i++){
    vector<int> verticalStrip;
    for(int j = 0; j<image.height(); j++){
      verticalStrip.push_back(-1);
    }
    distanceTable.push_back(verticalStrip);
  }
  ...
  vector<pair<int, RGBAPixel>> pattern = parseGaps(sgc);

  //first enque the starting point
  paths.enq(start);
  distanceTable[start.first][start.second] = 0;


  int count = 0;
  int d = pattern[count].first;
  while(!paths.empty()){    //deque and enque childiren
    pair<int, int> curr = paths.deq();
    while(d<distanceTable[curr.first][curr.second]){
      count = (count + 1) % pattern.size();
      d += pattern[count].first + 1; // add one foir the current colored pixel and then gap
    }

  //Another while loop with the condition that is false until the first in ur parse gaps is lesss tahn the Distance matrix ad curr.first, curr.second


  // PUTTING GOOD NEIGHBOURS IN q
  vector<pair<int, int>> nbrs = neighbors(curr);
  for(int i = 0; i<4; i++){
    if(good(image, distanceTable, curr, nbrs[i])){
        distanceTable[nbrs[i].first][nbrs[i].second] = distanceTable[curr.first][curr.second] + 1;
        paths.enq(nbrs[i]);
         //how to update curr - another for loop???
      }
    //distanceTable[nei.first][nei.second] = 0; //mark neignbors that are visited
    }



// IF SATEMENT TO CHANGE COLOR PIXEL AFTER FINDING GOOD NBGRS TO PREVENT test failure
  if(d == distanceTable[curr.first][curr.second]){
    *(image.getPixel(curr.first, curr.second)) = pattern[count].second;
  }

  }

    // for(pair<int, int> nei: neighbors(curr)){
    //   if(good(img, distanceTable, curr, nei)){
    //     path.enq(nei);
    //     curr = nei; //how to update curr - another for loop???
    //   }
    // distanceTable[nei.first][nei.second] = 0; //mark neignbors that are visited
    // }
  ...
  /*
  questions:
  Rainbow - check understanding:
    how to do color pattern when deq
    how to measure gap- what's (gap, color_1)
  Good func - D matrix -- do we need to create one in rainbow??
  */
